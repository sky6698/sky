...
CHANNEL_GROUPS = {
    '央视': [
        ...
        'cgtn', 'cgtnfy', 'cgtney', 'cgtnalby', 'cgtnxby', 'cgtnwyjl', 'cctvfyjc', 'cctvdyjc', 'cctvhjjc'
    ],
    '卫视': [
...
# 缓存有效期：playurl 120秒（原300秒，缩短至签名有效期附近）
CACHE_TTL = 120
...
class Spider(BaseSpider):
    ...
    def _init_session(self):
        ...
        # 记录 Session 创建时间，用于健康检查
        self._session_created_at = time.time()

    def _ensure_session(self):
        """检查 Session 状态，必要时重建（每 5 分钟或连接异常时）"""
        if not hasattr(self, '_session_created_at'):
            self._init_session()
            return
        # 每 5 分钟强制重建一次，释放旧连接
        if time.time() - self._session_created_at > 300:
            self._init_session()
            return
        # 简单探测，若连接异常则重建
        try:
            self.session.head('https://www.cctv.com', timeout=3)
        except Exception:
            self._init_session()
