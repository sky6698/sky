...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        multiUrls = []
        if flag == 'CCTV':
            # 获取多清晰度数组 [("清晰度名称","url"),...] 高到低
            qualityList = self.get_multi_m3u8(urlTxt=id)
            for qname, qurl in qualityList:
                multiUrls.append(f"{qname}${qurl}")
        else:
            try:
                html = self.webReadFile(urlStr=id, header=self.header)
                guid = self.get_RegexGetText(Text=html, RegexText=r'var\sguid\s*=\s*"(.+?)";', Index=1)
                qualityList = self.get_multi_m3u8(urlTxt=guid)
                for qname, qurl in qualityList:
                    multiUrls.append(f"{qname}${qurl}")
            except:
                multiUrls.append(f"自动${id}")
                parse = 1

        result["parse"] = parse  #1=嗅探,0=播放
        result["playUrl"] = "#".join(multiUrls)
        result["url"] = ""
        ...
        "filter": {
            ...
            "纪录片": [
                {"key": "datapd-channel", "name": "频道",
                 "value": [{"n": "全部", "v": ""}, {"n": "CCTV1 综合", "v": "CCTV{1 综合"}, {"n": "CCTV2 财经", "v": "CCTV{2 财经"},
                           {"n": "CCTV3 综艺", "v": "CCTV{3 综艺"}, {"n": "CCTV4 中文国际", "v": "CCTV{4 中文国际"},
                           {"n": "CCTV5 体育", "v": "CCTV{5 体育"}, {"n": "CCTV6 电影", "v": "CCTV{6 电影"},
                           {"n": "CCTV7 国防军事", "v": "CCTV{7 国防军事"}, {"n": "CCTV8 电视剧", "v": "CCTV{8 电视剧"},
                           {"n": "CCTV9 纪录", "v": "CCTV{9 纪录"}, {"n": "CCTV10 科教", "v": "CCTV{10 科教"},
                           {"n": "CCTV11 戏曲", "v": "CCTV{11 戏曲"}, {"n": "CCTV12 社会与法", "v": "CCTV{12 社会与法"},
                           {"n": "CCTV13 新闻", "v": "CCTV{13 新闻"}, {"n": "CCTV14 少儿", "v": "CCTV{14 少儿"},
                           {"n": "CCTV15 音乐", "v": "CCTV{15 音乐"}, {"n": "CCTV17 农业农村", "v": "CCTV{17 农业农村"}]},
                {"key": "datafl-sc", "name": "类型",
            ...
            "特别节目": [
                {"key": "datapd-channel", "name": "频道",
                 "value": [{"n": "全部", "v": ""}, {"n": "CCTV1 综合", "v": "CCTV{1 综合"}, {"n": "CCTV2 财经", "v": "CCTV{2 财经"},
                           ...
                           {"n": "CCTV15 音乐", "v": "CCTV{15 音乐"}, {"n": "CCTV17 农业农村", "v": "CCTV{17 农业农村"}]},
                {"key": "datafl-sc", "name": "类型",
                 "value": [{"n": "全部", "v": ""}, {"n": "全部", "v": "全部"}, {"n": "新闻", "v": "新闻"}, {"n": "经济", "v": "经济"},
                           {"n": "综艺", "v": "综艺"}, {"n": "体育", "v": "体育"}, {"n": "军事", "v": "军事"}, {"n": "影视", "v": "影视"},
                           {"n": "科教", "v": "科教"}, {"n": "戏曲", "v": "戏曲"}, {"n": "青少", "v": "青少"}, {"n": "音乐", "v": "音乐"},
            ...
            "节目大全": [{"key": "cid", "name": "频道",
                          ...
                          {"key": "year", "name": "年份",
                           "value": [{"n": "全部", "v": ""}, {"n": "2023", "v": "2023"}, {"n": "2022", "v": "2022"},
                                     ...
                                     {"n": "2000", "v": "2000"}]},
                          {"key": "month", "name": "月份",
                           "value": [{"n": "全部", "v": ""}, {"n": "12", "v": "12"}, {"n": "11", "v": "11"}, {"n": "10", "v": "10"},
                                     {"n": "09", "v": "09"}, {"n": "08", "v": "08"}, {"n": "07", "v": "07"}, {"n": "06", "v": "06"},
                                     {"n": "05", "v": "05"}, {"n": "04", "v": "04"}, {"n": "03", "v": "03"}, {"n": "02", "v": "02"},
                                     {"n": "01", "v": "01"}]}]
        }
    }
    header = {
    ...
    #【新增核心函数】获取全部清晰度，自动从高到低排序
    def get_multi_m3u8(self, urlTxt):
        #码率优先级：数字越大清晰度越高
        quality_map = {
            1800: "1080P",
            1200: "720P",
        ...
        priority = [1800, 1200, 800, 400, 240]
        valid_quality = []
        ...
        try:
            ...
            jo = json.loads(html)
            master_m3u8 = jo['hls_url'].strip()
            urlPrefix = self.get_RegexGetText(Text=master_m3u8, RegexText='(http[s]?://[a-zA-z0-9.]+)/', Index=1)

            #遍历所有码率，检测链接有效性
            for rate in priority:
                parts = master_m3u8.split('/')
                if len(parts) >= 4:
                    parts[3] = str(rate)
                    parts[-1] = f"{rate}.m3u8"
                    targetUrl = urlPrefix + '/'.join(parts)
                    status = self.TestWebPage(urlStr=targetUrl, header=self.header)
                    if status == 200:
                        valid_quality.append((quality_map[rate], targetUrl))
        except Exception as e:
            print("解析清晰度异常:", e)

        #兜底，如果没有获取到任何清晰度，返回原始链接
        if len(valid_quality) == 0:
            try:
                html = self.webReadFile(apiUrl, self.header)
                jo = json.loads(html)
                valid_quality.append(("默认", jo['hls_url']))
            except:
                pass
