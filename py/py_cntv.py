...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def get_m3u8(self,urlTxt):
		...
		# 已经是媒体播放列表（ts 片段）直接返回
		if link.endswith('.ts') or '.ts?' in link:
			return link
		content = self.webReadFile(urlStr=link, header=self.header).strip()
		...
		def build_url(path):
			if path.startswith('http'):
				...
				return link.rsplit('/', 1)[0] + '/' + path

		# 收集所有清晰度变体，并按 BANDWIDTH（带宽/清晰度）降序排列
		variants = []
			...
			if line.startswith('#EXT-X-STREAM-INF'):
				...
				if i + 1 < len(lines):
					...
					if path and not path.startswith('#'):
						variants.append((band, path))
		variants.sort(key=lambda x: x[0], reverse=True)

		if variants:
			# 优先返回可用的最高清晰度
			for band, path in variants:
				u = build_url(path)
				try:
					if self.TestWebPage(urlStr=u, header=self.header) == 200:
						return u
				except Exception:
					continue
			# 全部不可用时，兜底返回最高带宽对应的链接
			return build_url(variants[0][1])
		# 主播放列表中未找到变体，直接返回原始链接
		return link
	#搜索
