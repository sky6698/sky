...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def playerContent(self,flag,id,vipFlags):
		...
		if flag=='CCTV':
			# 这里返回【多条清晰度链接||分隔】
			multiUrls = self.get_m3u8_multi(urlTxt=id)
			if len(multiUrls) > 0:
				url = "||".join(multiUrls)
			else:
				url = ""
			...
			try:
				...
				multiUrls = self.get_m3u8_multi(urlTxt=guid)
				if len(multiUrls) > 0:
					url = "||".join(multiUrls)
				else:
					url = guid
	...
	config = {
		...
		{"key":"datadq-area","name":"地区","value":[{"n":"全部","v":""},{"n":"中国大陆","v":"中国大陆"},{"n":"美国","v":"美国"},{"n":"欧洲","v":"Europe"}]},
		{"key":"dataszm-letter","name":"字母","value":[{"n":"全部","v":""},{"n":"A","v":"A"},{"n":"C","v":"C"},{"n":"E","v":"E"},{"n":"F","v":"F"},{"n":"G","v":"G"},{"n":"H","v":"H"},{"n":"I","v":"I"},{"n":"J","v":"J"},{"n":"K","v":"K"},{"n":"L","v":"L"},{"n":"M","v":"M"},{"n":"N","v":"N"},{"n":"O","v":"O"},{"n":"P","v":"P"},{"n":"Q","v":"Q"},{"n":"R","v":"R"},{"n":"S","v":"S"},{"n":"T","v":"T"},{"n":"U","v":"U"},{"n":"V","v":"V"},{"n":"W","v":"W"},{"n":"X","v":"X"},{"n":"Y","v":"Y"},{"n":"Z","v":"Z"},{"n":"0-9","v":"0-9"}]}
		],
		"纪录片":[
	...
	# ============新增：获取全部清晰度链接，高到低排序 ============
	def get_m3u8_multi(self,urlTxt):
		#央视可用码率 从高到低
		quality_list = [
			{"name":"超清1200","rate":"1200"},
			{"name":"高清850","rate":"850"},
			{"name":"标清450","rate":"450"},
			{"name":"流畅200","rate":"200"}
		]
		...
		try:
			...
			if not urlPrefix:
				return []
			#解析主m3u8路径结构
			arr = master_m3u8.split('/')
			valid_urls = []
			for q in quality_list:
				new_arr = arr.copy()
				if len(new_arr)>=4:
					new_arr[3] = q['rate']
					new_arr[-1] = f"{q['rate']}.m3u8"
					play_url = urlPrefix + '/'.join(new_arr)
					#测试链接有效性
					rspCode = self.TestWebPage(urlStr=play_url,header=self.header)
					if rspCode == 200:
						valid_urls.append(f"{q['name']}${play_url}")
			return valid_urls
		except Exception as e:
			return []
