...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def webReadFile(self,urlStr,header):
		...
		return html
	#判断网络地址是否存在（修复：CNTV CDN 对 HEAD 一律返回 403，改用 GET+Range 只取响应头，不下载视频体）
	def TestWebPage(self,urlStr,header):
		html=''
		req=urllib.request.Request(url=urlStr,method='GET',headers={'User-Agent':self.header['User-Agent'],'Range':'bytes=0-64'})#,headers=header
		with  urllib.request.urlopen(req)  as response:
	...
	def removeHtml(self,txt):
		...
		txt =soup.sub('', txt)
		return txt.replace("&nbsp;"," ")
	#探测某 guid 视频真实可用的清晰度（修复：CDN 拒绝 HEAD 返回 403 改用 GET+Range；master 列表常只列 450 一档，需逐档探测）
	def probe_bits(self,guid):
		good=[]
		try:
			url="https://vdn.apps.cntv.cn/api/getHttpVideoInfo.do?pid={0}".format(guid)
			...
			subUrl=arr[-1].split('/')
			if len(subUrl)>4 and subUrl[1]=='asp':
				baseParts=subUrl[4:-1]# 例如 ['0303000a','3','default','guid']
			else:
				mm=re.search(r'asp/hls/main/(.+?)/default/([0-9a-f]+)/',link)
				baseParts=mm.group(1).split('/')+[mm.group(2)] if mm else []
			seen=set()
			for line in arr:
				mm=re.search(r'/(\d+)/',line)
				if mm:
					seen.add(mm.group(1))
			for b in ['2048','1200','850','450','270']:
				if b not in seen:
					seen.add(b)
			for b in ['2048','1200','850','450','270']:
				hdUrl=urlPrefix+'/'+'/'.join(['asp','hls',b]+baseParts+[b+'.m3u8'])
				try:
					rsp=self.TestWebPage(urlStr=hdUrl,header=self.header)
				except:
					rsp=''
				if rsp in (200,206):
					good.append((b,hdUrl))
			...
			pass
		return good
	#取m3u8（修复版：探测真实可用清晰度，prefer 指定优先档，缺省取最高清）
	def get_m3u8(self,urlTxt,prefer=''):
		if urlTxt and urlTxt.find('.m3u8')>0:
			return urlTxt
		good=self.probe_bits(urlTxt)
		if len(good)<1:
			return ''
		if prefer:
			for b,u in good:
